 ...
 *
 * @author Q
 * created at 2020/7/22 9:13
 */

public class OrderSearchAndExecuteFragment extends BaseFragment implements View.OnClickListener, BaseQuickAdapter.RequestLoadMoreListener{
    ...
    private void resetPatOrders(){
        ...
        orderSearcheTypeAdapter.setOnItemClickListener(new BaseQuickAdapter.OnItemClickListener() {
            ...
            public void onItemClick(BaseQuickAdapter adapter, View view, int position) {
                ...
                //左侧刷新分类选中状态显示
                orderSearcheTypeAdapter.setSelectedCode(sheetCode);
                orderSearcheTypeAdapter.notifyDataSetChanged();
            ...
            public void onSuccess(OrderSearchBean orderSearchBean) {
                ...
                if ("1".equals(pageNo)) {
                    ...
                    if (!"".equals(orderSearchBean.getSheetDefCode())) {
                        ...
                        orderSearcheTypeAdapter.setSelectedCode(orderSearchBean.getSheetDefCode());
                    }
                    orderSearcheTypeAdapter.setNewData(orderSearchBean.getSheetList());
                    if (!"".equals(orderSearchBean.getSheetDefCode())) {
                        for (int i = 0; i < orderSearchBean.getSheetList().size(); i++) {
                            if (orderSearchBean.getSheetList().get(i).getCode().equals(orderSearchBean.getSheetDefCode())) {
                                recyOrderexecuteOrdertype.scrollToPosition(i);
                                break;
                            }
                        }
                    }
                    patientPatsAdapter.setDetailColums(orderSearchBean.getDetailColums());
                    patientPatsAdapter.setNewData(orderSearchBean.getOrders());
                    recyOrderexecutePatorder.scrollToPosition(0);
                    if (orderSearchBean.getOrders().size() == 0) {
                        patientPatsAdapter.loadMoreEnd();
                    } else {
                        patientPatsAdapter.loadMoreComplete();
                    }
                } else {
                    if (orderSearchBean.getOrders().size() == 0) {
                        patientPatsAdapter.loadMoreEnd();
                    } else {
                        patientPatsAdapter.addData(orderSearchBean.getOrders());
                        patientPatsAdapter.loadMoreComplete();
                    }
                }
...
//                askCount = 0;
            ...
            public void onFail(String code, String msg) {
                askCount = 0;
                hideLoadingTip();
                patientPatsAdapter.loadMoreFail();
