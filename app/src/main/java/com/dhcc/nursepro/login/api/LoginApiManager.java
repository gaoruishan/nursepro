...
//                        callback.onFail("-2", "网络错误，数据解析失败");
    ...
    public static void getLoginConfig(final GetLoginConfigCallback callback) {
        LoginApiService.getLoginConfig(new LoginApiService.ServiceCallBack() {
            @Override
            public void onResult(String jsonStr) {
                ...
                if (jsonStr.isEmpty()) {
                    ...
                    try {
                        LoginConfigBean loginConfigBean = gson.fromJson(jsonStr, LoginConfigBean.class);
                        if (ObjectUtils.isEmpty(loginConfigBean)) {
                            callback.onFail("-3", "网络错误，数据解析为空");
                        } else {
                            if ("0".equals(loginConfigBean.getStatus())) {
                                if (callback != null) {
                                    callback.onSuccess(loginConfigBean);
                                }
                            } else {
                                if (callback != null) {
                                    callback.onFail(loginConfigBean.getMsgcode(), loginConfigBean.getMsg());
                                }
                            }
                        }
                    } catch (Exception e) {
//                        callback.onFail("-2", "网络错误，数据解析失败");
    ...
    public interface GetLoginConfigCallback extends CommonCallBack {
        void onSuccess(LoginConfigBean broadCastListBean);
    }
