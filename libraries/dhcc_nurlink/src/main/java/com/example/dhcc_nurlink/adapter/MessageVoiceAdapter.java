 ...
 * Time:15:16
 */
public class MessageVoiceAdapter extends BaseQuickAdapter<MessageListBean.MesListBean, BaseViewHolder>   implements MediaPlayer.OnBufferingUpdateListener,
    ...
    public MessageVoiceAdapter(@Nullable List<MessageListBean.MesListBean> data) {
        super(R.layout.item_nurlink_note, data);
    }
    @Override
    protected void convert(BaseViewHolder helper, MessageListBean.MesListBean item) {
        helper.setText(R.id.tv_datetime,item.getRecDateTime())
                .setText(R.id.patinfodetail_title,item.getMsgFromUserName()+"  ("+item.getMesStatus()+")")
                .addOnClickListener(R.id.tv_playvoice)
                .addOnClickListener(R.id.ll_message_rightmenu)
                .setGone(R.id.tv_playvoice,item.getMsgAddress().equals("0")?false:true);
...
//        btnPlay.performClick();
    ...
    public void onCompletion(MediaPlayer mp) {
        ...
        init(PATH);
        if (voiceFinishCallBack!=null){
            voiceFinishCallBack.finish();
        }
    ...
    public void setVoiceFinishCallBack(MessageVoiceAdapter.voiceFinishCallBack voiceFinishCallBack) {
